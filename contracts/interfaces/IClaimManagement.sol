 ...
 */
 interface IClaimManagement {
  ...
  function coverPoolClaims(address _coverPool, uint256 _nonce, uint256 _index) external view returns (
    ClaimState state,
    address filedBy,
  ...
  function fileClaim(address _coverPool, bytes32 _coverPoolName, uint48 _incidentTimestamp) external;
  function forceFileClaim(address _coverPool, bytes32 _coverPoolName, uint48 _incidentTimestamp) external;
  ...
  function decideClaim(address _coverPool, uint256 _nonce, uint256 _index, bool _claimIsAccepted, uint16 _payoutNumerator, uint16 _payoutDenominator) external;
