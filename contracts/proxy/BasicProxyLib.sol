...
library BasicProxyLib {
  ...
  */
  ...
  function computeProxyAddress(address runtimeCodeAddress, bytes32 salt, address deployer) internal pure returns (address) {
    return Create2.computeAddress(salt, keccak256(getProxyDeploymentCode(runtimeCodeAddress)), deployer);
  }
}