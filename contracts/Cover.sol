...
contract Cover is ICover, Initializable, ReentrancyGuard, Ownable {
  ...
  function mint(uint256 _receivedColAmt, address _receiver) external override onlyOwner {
    ...
    if (totalSupply == 0) {
      ...
      uint256 colTotal = totalCoverage * 1e18 / mintRatio;
      mintAmount = _receivedColAmt * totalSupply / colTotal;
    }
  ...
  function _collectFees() private {
    ...
    if (totalCoverage == 0) {
      ...
      uint256 fees = totalCoverageInCol * feeRate / 1e18;
      uint256 colToBePaidOut = totalCoverageInCol - fees;
