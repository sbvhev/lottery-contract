...
describe('CoverPool', () => {
  ...
  it('Should initialize correct state variables', async () => {
    const [collaterals, expiries, riskList, deletedRiskList, allCovers] = await coverPool.getCoverPoolDetails();

    expect(await coverPool.name()).to.equal(consts.POOL_2);
    expect(await coverPool.extendablePool()).to.equal(true);
    expect(await coverPool.poolStatus()).to.equal(1);
    expect(await coverPool.claimNonce()).to.equal(0);
    expect(await coverPool.noclaimRedeemDelay()).to.equal(3 * 24 * 60 * 60);
  ...
  it('Should update state variables by the correct authority', async () => {
    ...
    await coverPool.connect(ownerAccount).setPoolStatus(2);
    expect(await coverPool.poolStatus()).to.equal(2);
  ...
  it('Should NOT update state variables by the wrong authority', async () => {
    ...
    await expect(coverPool.connect(userAAccount).setPoolStatus(2)).to.be.reverted;
  ...
  it('Should add cover for userA and emit event', async () => {
    await expect(coverPool.connect(userAAccount).addCover(
      COLLATERAL, consts.ALLOWED_EXPIRYS[1], userAAddress,
  ...
  it('Should add cover for userB on existing contract', async () => {
    ...
    const txB = await coverPool.connect(userBAccount).addCover(
      COLLATERAL, consts.ALLOWED_EXPIRYS[1], userBAddress,
      ETHER_UINT_10, ETHER_UINT_10, '0x'
    );
    await txB.wait();
